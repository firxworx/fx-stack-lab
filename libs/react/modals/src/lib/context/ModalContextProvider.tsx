...
export const ModalContextProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  // @todo qa - release - added block as container is not valid `Element` per updated `container` prop type below
  if (!container) {
    return (
      <>
        {children}
  ...
  return (
    <>
      <ReactModalHookProvider container={container} rootComponent={ModalContainer}>
        {children}
